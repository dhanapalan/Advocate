...
Each extracted service: no `SUPABASE_SERVICE_ROLE_KEY` (ever); called directly from the browser
with the user's own Supabase access token — the same
...
### `FIELD_ENCRYPTION_KEY` is shared, not per-service

An earlier version of this document claimed each service got "its own independent
`FIELD_ENCRYPTION_KEY`" and presented that as a security property. That is not what shipped, and
the `wrangler.toml` comments in `matters/`, `diary/` and `drafting/` say so directly: all three
were **reunified with the main app's key** by a code-review fix, because each writes a column
another component reads back (`matters.notes`, `hearings.purpose`, `ai_drafts.content`). An
independent key there did not isolate anything — it silently made that data undecryptable
elsewhere.

So the real position, stated plainly:

- **One shared key** across the main Worker, the Supabase Edge Functions, and
  `matters`/`diary`/`drafting`. One leaked Worker secret exposes every encrypted column those
  cover.
- **`clients` alone has an independent key** — it is the one service whose encrypted column
  (`clients.notes`) nothing else reads, and it was stood up against zero pre-existing ciphertext.
- **`documents` and `assistant` have no key**, because neither owns an encrypted column.

Worth being equally plain about what this encryption is for. The key lives in the same Worker
that decrypts, and any authenticated colleague in the tenant reads plaintext through the app. It
protects against a raw database dump or Supabase-side exposure — not against a compromised
Worker, a stolen user session, or a malicious tenant member. That is still worth having; it just
